...
EEPROM.Remote = {}
EEPROM.Remote.FSBaseURL      = "https://localhost/remotefs"
EEPROM.Remote.CommonLib      = "/remoteCommonLib"    --- Used for common libraries to all computers no matter their specific purpose
EEPROM.Remote.DeveloperTools = "/developerTools"     --- Used for all kinds of debugging and the like
...
function EEPROM.Settings.ToComponentNickname( proxy, settings, lowerKeys )
    ...
    print("Writing nick to proxy " .. type(proxy))
...
local function getFirstDisk()
    local fsChildren = EEPROM.____filesystem.children( "/dev" )
    if #fsChildren == 0 then return '' end
    return fsChildren[ 1 ]
end
...
local function getRemoteServerCaps()
    ...
    local result, _, headers = request:await()
    result = ( result ~= nil )and( result >= 200 and result <= 299 )  -- Magic numbers are bad, 2xx is the request successful response range
        ...
        for i=1, #headers, 2 do
            ...
            if value == "Accept-Ranges" then
                ...
                if rangeBlockSize ~= nil then
                    print( "Remote accepts requests in chunks of " .. rangeBlockSize )
                end
                break
            end
        end
...
EEPROM.Boot.InternetCard            = computer.getPCIDevices( classes.FINInternetCard )[ 1 ]
...
EEPROM.Boot.UseTempFileSystem       = toboolean( EEPROM.Boot.ComputerSettings[ "usetempfilesystem" ], false ) and ( EEPROM.Boot.Disk == '' )
EEPROM.Boot.TempRemoteFiles         = toboolean( EEPROM.Boot.ComputerSettings[ "tempremotefiles" ], true )
EEPROM.Boot.AlwaysFetchFromRemote   = toboolean( EEPROM.Boot.ComputerSettings[ "alwaysfetchfromremote" ], false )
EEPROM.Settings.DebugMode           = toboolean( EEPROM.Boot.ComputerSettings[ "debugmode" ], false )
getRemoteServerCaps()
...
print ( "UseLoadFile      : " .. tostring( EEPROM.Boot.UseLoadFile ) )
...
function EEPROM.Remote.Fetch( remote, panicOnFail, debugTraceLevel, firstBlock, lastBlock )
    ...
    if firstBlock ~= nil and type( firstBlock ) == "number" and firstBlock >= 0
        and lastBlock ~= nil and type( lastBlock ) == "number" and lastBlock >= firstBlock
    then
...
function require( modname, remotefs, panicOnFailure )
    ...
    --     and EEPROM.____filesystem.exists( modname )
    -- then
    --     -- Load from filesystem and store as package
                ...
                local function buildPath( filepath )
                    ...
                    for i=1, #results - 1 do
                        ...
                        if  not EEPROM.____filesystem.exists( path )
                            and not EEPROM.____filesystem.createDir( path )
                        then
...
---Returns its version which must be the first line of its source file (see EEPROM.lua an example).  The variable name does not matter, nor does it matter if it's in a comment, just that a table of "{ integer, integer, integer, char }" appears on the first line.
...
---Downloads the remote copy of a remote file and returns its version which must be the first line of its source file (see EEPROM.lua an example).  The variable name does not matter, not does it matter if it's in a comment, just that a table of "{ integer, integer, integer, char }" (no quotes) appears on the first line.
---@return version, reason table, string returns nil, reason on any error and version, remote URL on success
function EEPROM.Version.GetRemoteEx( remote )
    ...
    function env.v( t )
        ...
        -- Input table can have 3 or 4 components
        ...
        --    or( #r4 == 1 and string.byte( r4 ) > 96 and string.byte( r4 ) < 123 )
        --)then
            -- Version will always have 4 components
            version = {
                math.floor( r1 ),
                math.floor( r2 ),
                math.floor( r3 ),
...
local function getMinorVersionCompare( v )
    local h = v[ 4 ] or ''
    local hotfix = 0
    if h ~= '' and #h == 1 then
...
function EEPROM.Version.Compare( a, b )
    if not EEPROM.Version.IsValid( a )
        or not EEPROM.Version.IsValid( b )
    then return nil end
    ...
    local va = getMinorVersionCompare( a )
    local vb = getMinorVersionCompare( b )
...
---Downloads the remote copy of the boot loader module and returns its version and remote URL or nil and an error string
...
---Downloads the remote copy of the EEPROM module and returns its version and remote URL or nil and an error string
...
-- This is meant for easily creating namespaced modules
-- This was largely copied from the Programming in Lua book
function createModule( f, v )
	local t = _G
